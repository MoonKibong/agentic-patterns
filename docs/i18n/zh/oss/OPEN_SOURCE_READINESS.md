...
Scope：repository docs、pattern catalog、GitHub template、i18n、contribution workflow 和公开定位。
...
| --- | --- | --- |
| Product promise | 用于 human-guided AI work 的 pattern language | Documented |
| License | CC BY 4.0 docs + MIT code | Present |
| README | Scope、catalog、validation、evidence hygiene | Present |
| Changelog | 带 `Unreleased` 的 Keep a Changelog 风格 | Present |
| Contributing | Pattern、evidence、i18n expectation | Present |
| Security | Private leakage 与 unsafe automation 报告 | Present |
| Code of conduct | 由 maintainer 执行的 lightweight policy | Present |
| Issue templates | 已有 pattern proposal；建议增加更多 template | Partial |
...
- 公开文档说明本项目是什么、不是什么。
- Maintainer 拥有 release 和 rollback 说明。
- Contributor 拥有 setup、validation、pattern、privacy 和 i18n expectation。
- 存在本地化的快速开始，并声明英文文档仍是最终权威。
- 无需 proprietary solution detail 即可理解或贡献 pattern。
